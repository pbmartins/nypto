...
def classification_gaussian_distribution(traffic_classes, pca_features,
    ...
    means = get_centroids(traffic_classes, n_obs, pca_features)
    covs = get_covariances(traffic_classes, n_obs, pca_features)
    traffic_idx = {}

    for i in range(n_obs):
        w = test_pca_features[i, :]
        probs = np.array([multivariate_normal.pdf(w, means[0], covs[0]),
                          multivariate_normal.pdf(w, means[1], covs[1]),
                          multivariate_normal.pdf(w, means[2], covs[2])])
        t_idx = np.argsort(probs)[-1]
        traffic_idx[i] = t_idx

        print('Obs: {:2}: Probabilities: [{:.4e},{:.4e},{:.4e}] -> '
...
def classification_clustering(traffic_classes, norm_pca_features,
    ...
    cluster_method = KMeans(init=centroids, n_clusters=3) \
    ...
    for i in range(norm_pca_test_features.shape[0]):
        ...
        print('Obs: {:2}: Probabilities beeing in each class: '
              '[{:.2f}%,{:.2f}%,{:.2f}%] -> Classification: {} -> {}'.format(
...
def classification_svm(traffic_classes, norm_features, norm_test_features, mode=0):
    ...
    print('class (from test PCA features with {}):'.format(modes[mode]['name']), l)
